...
public extension GetPresenter {
    struct Module: Cleanse.Module {
        public static func configure(binder: Binder<Singleton>) {
            ...
            binder.bindFactory(GamePresenter.self).with(GamePresenter.AssistedSeed.self).to { (gameRepository: Provider<GetGameRepository<GetGameLocalDataSource, GetGameRemoteDataSource, GameTransformer>>, seed: Assisted<GamesModel>) -> GamePresenter in
