...
final class GamePresenterTests: XCTestCase {
    ...
    private var presenter: GetPresenter<Int32, GamesModel, FakeGameInteractor>?
    ...
    func testGet() {
        presenter?.get()
        expect(self.presenter?.response?.id).toEventually(equal(GamesResponse.fakeGame.id))
...
extension GamePresenterTests {
    func injectProperties(_ localDataSource: FakeGetGameLocalDataSource, _ remoteDataSource: FakeGetGameRemoteDataSource, _ mapper: GameTransformer) {
        ...
        self.presenter = GetPresenter(useCase: FakeGameInteractor(repository: repository), request: GamesResponse.fakeGame.id, response: mapper.transformResponseToDomain(response: GamesResponse.fakeGame))
    }
